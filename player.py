...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if player == max_player:
            best = {'position': None, 'score': -math.inf} # each score should maximise (be larger)
        else:
        ...
        for possible_move in state.available_moves():
            ...
            # step 2: recuse using minimax to simulate a game after making that move
            sim_score = self.minimax(state, other_player)
            # step 3: undo the move
            state.board[possible_move] = ' '
            state.current_winner = None
            sim_score['position'] = possible_move # otherwise this will get messed ud from the recursion
            # step 4: update the dictionaries if necessary
            if player == max_player: # we're trying to maximise the max_player
